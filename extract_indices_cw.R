...
for (index in indices){
  for (j in c(1:length(scenarios))){
    ...
    for (i in 1:length(gcmList)){
      ...
      data=fromJSON(paste0("http://maprooms.ciat.cgiar.org/climatewizard/service?lat=",lat,"&lon=",lon,"&index=",index,"&scenario=",scenarios[j],"&gcm=",gcm,"&range=",period,"&avg=false",mon))
      out <- data$values
